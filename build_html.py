...
function matchesSearch(d, q) {
  ...
  // 搜索性味
  if (d.nature && d.nature.toLowerCase().includes(q)) return true;
  
  // 搜索归经
  if (d.meridian && d.meridian.toLowerCase().includes(q)) return true;
  
  // 搜索功效
  if (d.effect && d.effect.toLowerCase().includes(q)) return true;
  
  // 搜索主治
  if (d.indication && d.indication.toLowerCase().includes(q)) return true;
  
  // 搜索组成（方剂的药材名）
  if (d.composition) {
    for (let i = 0; i < d.composition.length; i++) {
      if (d.composition[i].herb && d.composition[i].herb.toLowerCase().includes(q)) return true;
    }
  }
...
searchBox.addEventListener("input", function() {
  ...
  if (!q) {
    node.style("display", "block");
    link.style("display", "block");
    linkLabel.style("display", d => d.role ? 0.7 : 0);
    updateStats();
    return;
  }
  node.style("display", d => matchesSearch(d, q) ? "block" : "none");
  link.style("display", function(d) {
    const sId = typeof d.source === "object" ? d.source.id : d.source;
    const tId = typeof d.target === "object" ? d.target.id : d.target;
    const sEl = node.filter(n => n.id === sId);
  ...
  linkLabel.style("display", function(d) {
    ...
    const sEl = node.filter(n => n.id === sId);
    const tEl = node.filter(n => n.id === tId);
    return (sEl.style("display") !== "none" && tEl.style("display") !== "none" && d.role) ? 0.7 : 0;
  });
  updateStats();
});
